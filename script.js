...
// Обновленный метод отрисовки лонгридов с живым выкачиванием глоссария из облака
async function showSectionContent(section) {
    ...
    try {
        // Запрашиваем термины глоссария напрямую из Supabase
        const { data: cloudGlossary } = await supabaseClient.from('glossary').select('term, definition');
        
        if (cloudGlossary && cloudGlossary.length > 0) {
            // Сортируем от длинных к коротким, чтобы избежать багов вложенности слов
            const sortedGlossary = [...cloudGlossary].sort((a, b) => b.term.length - a.term.length);

            sortedGlossary.forEach(item => {
                const term = item.term.trim();
                const definition = item.definition.replace(/"/g, '&quot;'); // Экранируем кавычки для тултипа

                // Регулярное выражение ищет точное совпадение слова в тексте
                const regex = new RegExp(`\\b${term}\\b|(?<=\\s|^)${term}(?=\\s|[.,!?;:-]|$)`, 'gi');
                
                processedContent = processedContent.replace(regex, (match) => {
                    return `<span class="wiki-term" data-tooltip="${definition}">${match}</span>`;
                });
            });
        }
    } catch (err) {
        console.error("Ошибка применения облачного глоссария:", err.message);
    }
    
    // Выводим заголовок статьи и обработанный текст с подсказками
