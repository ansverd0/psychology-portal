...
let appDatabase = {}; 
let appGlossary = {}; // Сюда динамически загрузятся все 300+ терминов из Supabase
...
// Установка индикаторов ожидания на экранах до полной загрузки сети
function showLoadingStates() {
    if (termText) termText.innerText = "⏳ Загрузка лаборатории...";
    if (definitionText) definitionText.innerText = "Пожалуйста, подождите, собираем карточки из облака...";
    if (document.getElementById('quiz-question')) {
        document.getElementById('quiz-question').innerText = "⏳ Загрузка тестов... Пожалуйста, подождите.";
    }
    // Заглушка для пустого экрана разделов психологии (Пункт 8 багрепорта)
...
async function loadDatabase() {
    showLoadingStates(); // Включаем статус загрузки
    
    try {
        console.log("📡 Подключение к Supabase и сборка базы данных...");
        
        appDatabase = {
            flashcards: [], tickets: [], tests: [], sections: [], library: [], news: []
        };
        ...
        if (cards) {
            appDatabase.flashcards = cards.map(c => ({
                term: c.term, definition: c.definition, discipline: c.discipline_id 
            }));
        }
        ...
        if (qz) {
            appDatabase.tests = qz.map(t => ({
                question: t.question, options: t.options, correct: t.correct_index, link: t.link_url, discipline: t.discipline_id
            }));
        }

        // 3. Выкачиваем билеты с привязкой литературы (ИСПРАВЛЕНО ОТОБРАЖЕНИЕ КНИГ)
        ...
        if (tk) {
            ...
            // Выкачиваем связи Many-to-Many для литературы билетов
            const { data: links } = await supabaseClient.from('ticket_library').select('ticket_id, book_id');
            const { data: allBooks } = await supabaseClient.from('library').select('id, title, author');
        ...
        if (art) {
            ...
            Object.keys(secGroups).forEach(key => {
                secGroups[key].articles.sort((a, b) => (parseInt(a.title, 10) || 0) - (parseInt(b.title, 10) || 0));
            });
        ...
        // 6. Выкачиваем ченджлог новостей
        ...
        // 7. Динамически загружаем ГЛОССАРИЙ из облака для Вики-эффекта
        const { data: gloss } = await supabaseClient.from('glossary').select('term, definition');
        if (gloss) {
            gloss.forEach(g => {
                appGlossary[g.term.trim()] = g.definition;
            });
        }

        console.log("✅ База данных Supabase успешно скомпилирована под движок сайта!");

        // Вызовы модулей инициализации интерфейса
        ...
        if (document.getElementById('sections-page-marker')) initSections();
...
function showSectionContent(section) {
    ...
    // Пробегаемся по динамическому облачному словарю appGlossary
    Object.keys(appGlossary).forEach(term => {
        const regex = new RegExp(`\\b${term}\\b|(?<=\\s|^)${term}(?=\\s|[.,!?;:-]|$)`, 'gi');
        processedContent = processedContent.replace(regex, (match) => {
            return `<span class="wiki-term" data-tooltip="${appGlossary[term]}">${match}</span>`;
        });
    });
